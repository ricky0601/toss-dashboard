import type { InvestmentAccountStatus, PortfolioOverview } from "@/lib/dashboard/types";
import { DashboardIcon } from "./dashboard-icon";

type TotalAssetsWidgetProps = {
  readonly overview: PortfolioOverview;
  readonly accountStatus: InvestmentAccountStatus;
};

function accountStatusText(status: InvestmentAccountStatus): string {
  switch (status.kind) {
    case "api":
      return `${status.primaryAccountType} 계좌 확인`;
    case "mock":
      return "API 연결 실패 시 mock 유지";
  }
}

export function TotalAssetsWidget({ overview, accountStatus }: TotalAssetsWidgetProps) {
  return (
    <section id="overview" aria-labelledby="overview-title" className="overflow-hidden rounded-widget bg-primary p-6 text-white sm:p-8 lg:p-10">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div className="min-w-0">
          ...
          <p className="break-words text-asset font-bold">{overview.totalAssets}</p>
          <div className="mt-5 flex flex-wrap items-center gap-2 text-body-small">
            ...
            <span className="font-bold text-gain-on-dark">{overview.dailyChange}</span>
            <span className="font-bold text-gain-on-dark">({overview.dailyRate})</span>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-medium bg-white/10 p-4 lg:min-w-56">
          ...
          <div>
            ...
            <p className="mt-1 text-body-small font-semibold text-white">{overview.updatedAt}</p>
            <p className="mt-1 text-caption font-semibold text-white/50">{accountStatusText(accountStatus)}</p>
          </div>
        </div>
      </div>
    </section>
